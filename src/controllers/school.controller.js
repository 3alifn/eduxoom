...
const app = express();
const { sqlmap, randomBytes, createHmac, path, fs, nodemailer, dotenv} = require('../configs/server');
       ...
       const filename= req.file.filename;
        sqlmap.query(
            ...
            (err, info) => {
                if (info.length > 0) {
                    sqlmap.query(
                        `UPDATE school_settings SET image=? WHERE domain=?`,
                        [filename, res.locals.hostname],
                        (err, next) => {
                    ...
                    sqlmap.query(
                        `INSERT INTO school_settings (domain, image) VALUES(?, ?)`,
                        [res.locals.hostname, filename],
                        (err, next) => {
                            if (err) console.log(err.sqlMessage);
