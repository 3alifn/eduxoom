...
const { public_gallery_video, public_gallery_image, public_gallery_video_list, public_gallery_image_list, public_carousel_get } = require("../app/content")
...
const { public_result_get } = require("../app/result_app")
const { public_routine_page, public_routine_page_class_base, public_routine_download } = require("../app/routine_app")
const { public_student_page, public_student_profile, public_student_list, self_join_student, self_student_verify_code, self_student_send_mail } = require("../app/student_app")
...
public.get("/about/us/team", (req, res) => {
  res.render("public/team_our")
})
...
// student router.........
public.get("/join/student", (req, res)=>{
    res.render("student/join_student", {step: "step1", msg: req.flash("msg"), alert: req.flash("alert")})
  })
  public.post("/join/student/sendmail", self_student_send_mail)
  public.post("/join/student/verifycode", self_student_verify_code)
  
public.post("/join/student", self_join_student)
public.get("/all/student", public_student_list)
public.get("/student/page", public_student_page)
public.get("/student/result", (req, res)=>{

  res.render("result/result_page_public")
  })

public.post("/result/get", public_result_get)
...
// result router......
public.get("/result/page/", (req, res)=>{
  res.render('result/result_page_public')
})
public.post("/result/get", public_result_get)
