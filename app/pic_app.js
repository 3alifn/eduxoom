          ...
          (errfound, subjectfound) => {
              if (subjectfound.length > 0) {
                  ...
                  sqlmap.query(
                      ...
                      [req.hostname, className, sectionName],
                      (err_row, count_row) => {
                          if (err_row) {
                          ...
                          sqlmap.query(
                              ...
                              [req.hostname, className, sectionName, limit, offset * limit],
...
exports.teacher_pic_mark_post = (req, res) => {
  ...
  sqlmap.query(
      ...
      [req.hostname, className, sectionName, student_uuid, subject_code, chapter],
      (errCheck, infoCheck) => {
          if (errCheck) {
          ...
          if (infoCheck === undefined || infoCheck.length === 0) {
              sqlmap.query(
                  ...
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                  [req.hostname, session, className, sectionName, subject_flag, subject_code, pi_group, student_uuid, subject, roll, name, avatar, chapter, pi, checkout, bg_color],
                  (errPost, nextPost) => {
                      if (errPost) {
...
exports.teacher_pic_mark_checkout = (req, res) => {
  ...
  sqlmap.query(
      ...
      [req.hostname, className, sectionName, subject_code],
