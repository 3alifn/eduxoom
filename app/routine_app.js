...
exports.admin_subject_dynamic_get= (req, res)=>{
    ...
    sqlmap.query(`SELECT subject FROM subject WHERE class="${className}" AND section IN ("ALL", "${sectionName}") GROUP BY subject ORDER BY ID DESC`, (err, info)=>{
        if(err) console.log(err.sqlMessage);
...
exports.admin_teacher_dynamic_get= (req, res)=>{
    ...
    sqlmap.query(`SELECT name, pds_id FROM teachers ORDER BY ID`, (err, info)=>{
        if(err) console.log(err.sqlMessage);
