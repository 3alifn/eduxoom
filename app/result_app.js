...
exports.dynamic_roll_list= (req, res)=>{
...
exports.dynamic_subject_list = (req, res)=>{
 ...
 sqlmap.query(`SELECT subject FROM subject WHERE class="${className}" AND section="${sectionName}"`, (err, info)=>{
 ...
 let xlsxFilePath= './public/docs/result/'+req.file.filename;
  ...
  for(let run=0; run<xlsxImportData.length; run++){
    ...
    for (let index = 0; index < markData[0].length; index++) {
     ...
     let sql = `INSERT INTO result (class, section, student_id, name, roll,  subject, marks) VALUES ( "${className}", "${sectionName}", "${xlsxImportData[run].student_id}", "${xlsxImportData[run].name}", "${xlsxImportData[run].roll}",   "${Object.keys(xlsxImportData[run]).slice(3)[index]}", "${Object.values(xlsxImportData[run]).slice(3)[index]}")`
...
exports.admin_result_rank_post= (req, res)=>{
  ...
  sqlmap.query(`SELECT student_id FROM result GROUP BY student_id ORDER BY student_id `, (err, info)=>{
...
exports.admin_result_post = (req, res) => {
 ...
 for (let i=0; i< subject.length; i++) {
  sqlmap.query(`INSERT INTO result (class, section, student_id,  marks, subject) VALUES ("${className}", "${section}", "${student_id}", "${mark[i]}", "${subject[i]}")`, (err, next) => {
...
exports.admin_result_rank_list = (req, res)=>{
  ...
  sqlmap.query(`SELECT * FROM result WHERE class="${className}" AND section="${sectionName}" GROUP BY student_id ORDER BY rank DESC, roll`, (errRank, info)=>{
...
exports.public_result_get = (req, res) => {
  ...
  sqlmap.query(`SELECT name, student_id, class, section, avatar FROM result WHERE class='${className}' AND section='${section}'  AND student_id='${student_id}'`, (errStudent, infoStudent)=>{
        ...
        if (errOne) console.log(errOne.sqlMessage);
          ...
          let sql = `SELECT class, section, student_id, marks, subject, CASE 
           ...
           ELSE "0.00"
            ...
            END AS points
             FROM result WHERE class="${className}" AND section="${section}" AND student_id="${student_id}" ORDER BY subject`
    ...
    <center><span class=" badge bg-dark text-light">  <img src="/image/school/bga.png" height="40px" class=" rounded"  width="40px" alt="404"> Boalmari George Academy</span></center>
    <br>
   <center>
...
sqlmap.query(`SELECT student_id, rank FROM result GROUP BY student_id ORDER BY rank DESC, roll`, (errRank, infoRank)=>{
  if(errRank) console.log(errRank.sqlMessage);
