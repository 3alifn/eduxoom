...
exports.teacher_attendance_post = (req, res) => {
  ...
  sqlmap.query(`SELECT * FROM attendance WHERE student_id=${student_id} AND find_date="${findDate}"`, (errr, next) => {
    ...
    if (errr) console.log(errr.sqlMessage)
      ...
      if (next.length > 0) {
        ...
        sqlmap.query(`UPDATE attendance SET at_status="${status}" WHERE student_id=${student_id} AND session="${session}" AND month="${Month}" AND day="${Day}"`, (err, next) => {
  ...
  sqlmap.query(`INSERT INTO attendance (find_date, at_find_date, class, section, name, avatar, student_id, roll, student_uuid, at_status, session, month, day ) VALUES("${findDate}",  "${atFindDate}",  "${className}", "${sectionName}", "${studentName}", "${avatar}",  "${student_id}", "${roll}", "${ID}", "${status}", "${session}", "${Month}", "${Day}")`, (err, next) => {
...
exports.teacher_attendance_page = (req, res) => {
  ...
  sqlmap.query(`SELECT * FROM students WHERE class="${className}" AND section="${sectionName}" GROUP BY student_id  ORDER BY roll`, (err, info) => {
    ...
    if (err) console.log(err.sqlMessage)
      ...
      let Month = new Date().toDateString().substring(4, 7)
      let Day = new Date().toDateString().substring(0, 4) + new Date().getDate()
      sqlmap.query(`SELECT * FROM attendance WHERE class="${className}" AND section="${sectionName}" AND session=${session} AND month="${Month}" AND day="${Day}" GROUP BY student_id  ORDER BY roll`, (err_sub, info_sub) => {
...
exports.privet_attendance_page = (req, res) => {
  ...
  let sql = `SELECT * FROM attendance WHERE class='${className}' AND section='${sectionName}' AND session='${session}' AND month='${Month}'   GROUP BY student_id ORDER BY day`
...
exports.privet_attendance_page_class_base= (req, res) => {
  ...
  let sql = `SELECT * FROM attendance WHERE class='${className}' AND section='${sectionName}' AND session='${session}' AND month='${Month}'  GROUP BY student_id ORDER BY roll `
    ...
    else 
    {
      ...
      for (const i in info) {
        ...
        <hr>
                  ...
                  <a href="/privet/attendance/report/history?student_id=${info[i].student_id}&session=${info[i].session}&month=${info[i].month}" class="badge bg-light text-dark">History</a>
...
exports.privet_attendance_report_history= (req, res)=>{
  ...
  let {student_id, session, month} = req.query;

  sqlmap.query(`SELECT * FROM attendance WHERE  session="${session}" AND month="${month}" AND student_id=${student_id}`, (err, info)=>{
    ...
    else 
    {
...

      {
        ...
        sqlmap.query(`SELECT COUNT(at_status) as present FROM attendance WHERE  session="${session}" AND month="${month}" AND student_id=${student_id} AND at_status="Present"`, (errP, infoP)=>{

          if(errP) console.log(errP.sqlMessage);
      
          sqlmap.query(`SELECT COUNT(at_status) as absent FROM attendance WHERE  session="${session}" AND month="${month}" AND student_id=${student_id} AND at_status="Absent" `, (errA, infoA)=>{
      
            if(errA) console.log(errA.sqlMessage);
      
      
            sqlmap.query(`SELECT * FROM attendance WHERE session="${session}" AND month="${month}" AND student_id=${student_id} ORDER BY ID`, (errS, infoS)=>{
  ...
  sqlmap.query(`SELECT * FROM attendance WHERE session="${session}" AND month="${month}" AND student_id=${student_id} `, (err, info)=>{
    ...
    else 
    {
...

      {
        ...
        sqlmap.query(`SELECT at_status, day, find_date FROM attendance WHERE  session="${info[0].session}" AND month="${info[0].month}" AND student_id=${info[0].student_id} AND at_status="Present" ORDER BY ID`, (errP, infoP)=>{

          if(errP) console.log(errP.sqlMessage);
      
          sqlmap.query(`SELECT at_status, day, find_date FROM attendance WHERE  session="${info[0].session}" AND month="${info[0].month}" AND student_id=${info[0].student_id} AND at_status="Absent" ORDER BY ID`, (errA, infoA)=>{
      
            if(errA) console.log(errA.sqlMessage);
      
      
            sqlmap.query(`SELECT * FROM attendance WHERE session="${session}" AND month="${month}" AND student_id=${info[0].student_id} ORDER BY ID DESC `, (errS, infoS)=>{
