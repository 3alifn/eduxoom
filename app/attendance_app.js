...
exports.teacher_attn_post_page = (req, res) => {
    ...
    sqlmap.query(
      `SELECT ID, student_id, student_uuid FROM students 
      WHERE domain=? AND class=? AND section=? 
      GROUP BY student_id 
      ORDER BY roll`, 
      ...
      (errS, infoS) => {
        ...
        sqlmap.query(
          `SELECT ID, student_id, student_uuid, avatar, name, roll FROM students 
          WHERE domain=? AND class=? AND section=? 
          GROUP BY student_id 
          ORDER BY roll 
          LIMIT 20 OFFSET 0`, 
  ...
  exports.teacher_attn_post = (req, res) => {
    ...
    sqlmap.query(
        `SELECT user_id, find_date, attn_date FROM attn_record WHERE domain=? AND user='Student' AND user_id=? AND find_date=? ORDER BY id DESC`, 
        [req.hostname, student_id, find_date], 
        (errf, find) => {
            if (errf) {
            ...
            if (find.length > 0) {
                sqlmap.query(
                    `UPDATE attn_record SET checkout=?, at_status=? WHERE domain=? AND user='Student' AND user_id=? AND find_date=?`, 
                    [checkout, at_status, req.hostname, student_id, find_date], 
                    (erru, up) => {
                        if (erru) {
                ...
                sqlmap.query(
                    ...
                    VALUES (?, ?, ?, ?, 'Student', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, 
                    [req.hostname, currentYear, duplicate_data, 1, get_cal, attn_date, find_date, checkout, at_status, class_name, section_name, student_id, name, roll, avatar, currentYear, currentMonth, currentDate], 
                    (erri, inser) => {
                        if (erri) {
...
exports.teacher_attn_checkout = (req, res) => {
    ...
    sqlmap.query(
        `SELECT user_id, checkout, attn_date FROM attn_record WHERE domain=? AND user='Student' 
        AND class=? AND section=? AND attn_date=?`, 
...
exports.teacher_attn_checkout_last_five = (req, res) => {
    ...
    sqlmap.query(
        `SELECT user_id, checkout, attn_date FROM attn_record WHERE domain=? AND class=? AND section=? 
        AND user_id=? ORDER BY attn_date LIMIT 5`, 
    ...
    sqlmap.query(
        ...
        (errf, infof) => {
            ...
            if (infof.length > 0) {
                ...
                sqlmap.query(
                    `SELECT id, user_id, avatar, name, roll, checkout, at_status FROM attn_record
                    WHERE domain=? AND class=? AND section=? AND attn_date=? GROUP BY user_id ORDER BY roll LIMIT 20 OFFSET 0`,
...
exports.privet_attn_repo_page_num = (req, res) => {
    ...
    sqlmap.query(
        `SELECT id, user_id, avatar, name, checkout, at_status, roll FROM attn_record
        WHERE domain=? AND class=? AND section=? AND attn_date=?
        GROUP BY user_id ORDER BY roll LIMIT 20 OFFSET ?`, 
...
exports.privet_attn_repo_find = (req, res) => {
    ...
    sqlmap.query(
        `SELECT id, user_id, avatar, name, checkout, at_status, roll FROM attn_record
        WHERE domain=? AND class=? AND section=? AND attn_date=?
        GROUP BY user_id ORDER BY roll LIMIT 20 OFFSET 0`, 
        [req.hostname, class_name, section_name, attn_date],
        (err, info) => {
            if (err) {
                console.log(err.sqlMessage);
...
exports.privet_attn_student_calendar = (req, res) => {
    ...
    sqlmap.query(
        `SELECT id, user_id, avatar, name, roll, checkout, at_status FROM attn_record
        WHERE domain=? AND class=? AND section=? AND user_id=?`, 
        [req.hostname, class_name, section_name, student_id],
        (err, info) => {
            if (err) {
                console.log(err.sqlMessage);
...
exports.privet_attn_calendar_checkout = (req, res) => {
    ...
    sqlmap.query(
        `SELECT COUNT(CASE checkout WHEN 1 THEN 1 ELSE null END) AS present, 
                COUNT(CASE checkout WHEN 0 THEN 1 ELSE null END) AS absent 
         FROM attn_record 
         WHERE domain=? AND class=? AND section=? AND user_id=? AND get_cal=?`,
        ...
        (errc, cal) => {
            ...
            sqlmap.query(
                `SELECT find_date, get_cal, checkout 
                 FROM attn_record 
                 WHERE domain=? AND class=? AND section=? AND user_id=? AND get_cal=? 
                 ORDER BY day`,
                [req.hostname, class_name, section_name, student_id, get_cal],
...
function student_rank_mark_attn(domain, class_name, section_name, student_id, at_status) {
    sqlmap.query(
        `SELECT ?? FROM student_rank WHERE domain=? AND class=? AND section=? AND student_id=? ORDER BY ?? DESC LIMIT 1`,
        [at_status, domain, class_name, section_name, student_id, at_status],
        (errf, infof) => {
            if (errf) {
            ...
            if (infof.length > 0) {
                ...
                sqlmap.query(
                    `UPDATE student_rank SET ??=? WHERE domain=? AND class=? AND section=? AND student_id=?`,
                    [at_status, marked, domain, class_name, section_name, student_id],
                    (erru, infou) => {
                        if (erru) {
