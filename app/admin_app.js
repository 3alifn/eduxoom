...
admin_login: (req, res)=>{
    ...
    sqlmap.query(`SELECT * FROM user_admin WHERE hash_username='${hashUsername}' AND hash_password='${hashPassword}'`, (err, info)=>{
        ...
        if(info.length>0){
            ...
            if(info[0].hash_username===hashUsername && info[0].hash_password===hashPassword){
              ...
              res.redirect("/admin/cpanel/login")
...
self_password_update: (req, res)=>{
    ...
    let {hash_password, pastPassword}= req.body;
        ...
        let sql= `UPDATE user_admin SET hash_password="${hash_password}"`
     ...
     sqlmap.query(`SELECT hash_password FROM user_admin`, (errPass, infoPass)=>{
      ...
      if(errPass) console.log(errPass.sqlMessage);
        ...
        if( pastPassword==infoPass[0].hash_password)
  {
